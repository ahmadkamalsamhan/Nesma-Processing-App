...
PDF_DPI = 150
MAX_CHARS_PER_QUERY = 1000  # AI context limit per query
...
# Streamlit UI
# =========================
st.title("📄 Nesma PDF On-the-Fly OCR + AI Chat")

choice = st.radio("Select input:", ["🔗 Google Drive Link", "📂 Upload PDFs"])
...
if choice == "📂 Upload PDFs":
    ...
    if uploaded_files:
        for pdf in uploaded_files:
            ...
            page_input = st.text_input(f"Pages to process for {pdf.name} (e.g., 1,3-5, default all):", "all")
            ...
            if pages_selected:
                ...
                if st.button(f"Ask AI about {pdf.name}"):
                    context = text[:MAX_CHARS_PER_QUERY]
    ...
    if drive_link:
        ...
        if not drive_id:
            st.error("Invalid Google Drive link. Use a file or folder link.")
        else:
            files_to_process = []
            if link_type == "file":
